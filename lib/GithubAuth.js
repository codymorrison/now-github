...
module.exports = class GithubAuth {
   ...
   */
  constructor(clientId, clientSecret, { githubUrl = "github.com" } = {}) {
    ...
    // The universal ID to prevent CSRF attacks on our response callback.
    this.clientState = null;

    // Setup known routes for Github
  ...
  clientRedirect(res, params) {
    const location = `${this.redirectUrl}?${querystring.stringify(params)}`;
  ...
  async getAccessToken(code, state, redirect_uri) {
    ...
    if (!state || state !== this.clientState) {
   ...
   * @param [req.query.login] {String} - Suggests a specific account to use for signing in and authorizing the app.
  ...
  async login(req, res) {
    ...
    const { scope, allow_signup, redirect_uri, login } = req.query;

    // Create URL friendly universal ID to verify auth callback (Required by GH).
    this.clientState = await uid(20);
    ...
    console.log("login redirect: ", query.redirect_uri);
